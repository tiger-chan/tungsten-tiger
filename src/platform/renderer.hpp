...
namespace tt {
	...
	struct renderer_manager {
		...
		virtual void end() = 0;
		virtual void submit(const shader_program_handle & program, uint16 view, uint32 depth = 0, uint8 flags = UINT8_MAX) = 0;
		...
		virtual vertex_buffer_handle create_vertex_buffer(const void *mem, uint32 size, const vertex_layout &layout, vertex_buffer_flags = VB_NONE) = 0;
